 ...
 * The landing page.
 *
 * Restyled onto the design system, but its job is unchanged: prove the frontend
 * can reach the Django API, and state plainly what the system can and cannot
 * currently do. The two honesty notices at the bottom are the point of the page
 * and must not be quietly dropped when it gets prettier - while the engine is a
 * placeholder or no verified rules are loaded, the system cannot produce a real
 * finding, and saying so here is what stops a demonstration implying otherwise.
 */
  ...
  return (
    <section className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Packaged commodity compliance</h1>
          <p className="page-lede">
            Upload a photograph of a packaged product label to have its
            declarations extracted and checked against the compliance rules
            loaded in this system.
          </p>
        </div>
      ...
      {/*
        Two notices that must not be quietly removed once the UI gets prettier.
        While the engine is a placeholder or no verified rules are loaded, the
        system cannot produce a real finding, and saying so here is what stops a
        demo from implying otherwise.
      */}
      {data?.extractionEngine.isPlaceholder && (
        <p className="panel panel--warning">
          <strong>No OCR engine is installed.</strong> The extraction pipeline is
          wiring only: it reads no text from images and produces no label data.
          Results shown anywhere in this system are not real readings.
        </p>
      )}
